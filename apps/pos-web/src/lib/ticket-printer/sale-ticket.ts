...
import type { TicketPrintInput } from './types';
import { escapeHtml, formatDateTimeParts, formatStatusLabel, paymentMethodLabel, taxModeLabel } from './format';
    ...
    .map((item) => {
      return `<tr>
        ...
        <td>
          ...
          ${item.notes ? `<div class="item-meta" style="font-style: italic;">Nota: ${escapeHtml(item.notes)}</div>` : ''}
        </td>
