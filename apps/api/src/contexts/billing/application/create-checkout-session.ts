...
export async function createCheckoutSession(db: Kysely<Database>, input: CreateCheckoutInput) {
  return TracerHelper.withSpan('billing', 'billing.checkout.create', {
    [SemanticAttributes.TENANT_ID]: input.tenantId,
    [SemanticAttributes.GATEWAY_NAME]: input.gateway,
    [SemanticAttributes.PLAN_ID]: input.planId
  }, async (span) => {
    ...
    span.setAttribute(SemanticAttributes.AMOUNT_CENTS, plan.price_cents);
    ...
    await db
      ...
      .values({
        ...
        status: 'PENDING',
        metadata_json: { planId: input.planId, checkoutUrl, autoRenew: input.autoRenew }
      })
      .execute();
