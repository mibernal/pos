...
export class TablesRepository {
  ...
  async createRoom(tenantId: string, branchId: string, payload: CreateRoomPayload): Promise<Room> {
    const id = randomUUID();
    const result = await this.db.insertInto('rooms')
      .values({
        id,
        tenant_id: tenantId,
        branch_id: branchId,
  ...
  async getRoomsWithTables(tenantId: string, branchId: string): Promise<RoomWithTables[]> {
    const rooms = await this.db.selectFrom('rooms')
    ...
    if (tableIdsWithOrders.length > 0) {
      const activeOrders = await this.db.selectFrom('tables as t')
  ...
  async createTable(tenantId: string, branchId: string, roomId: string, payload: CreateTablePayload): Promise<Table> {
    const id = randomUUID();
    const result = await this.db.insertInto('tables')
      .values({
        id,
        tenant_id: tenantId,
        branch_id: branchId,
